...
var createHistorySlice = (set, get) => {
  ...
  return {
    ...
    back: () => {
      var _a;
      const { history, dispatch: dispatch2 } = get();
      if (history.hasPast()) {
    ...
    forward: () => {
      ...
      if (history.hasFuture()) {
        ...
        dispatch2({ type: "set", state: state ? tidyState(state) : {} });
        set({ history: __spreadProps(__spreadValues({}, history), { index: history.index + 1 }) });
      }
    },
    setHistories: (histories) => {
      var _a;
      const { dispatch: dispatch2, history } = get();
      dispatch2({
        type: "set",
  ...
  subscribeWithSelector2((set, get) => {
    ...
    return __spreadProps(__spreadValues({
      ...
      setUi: (ui, recordHistory) => set((s) => {
        const dispatch2 = createReducer({
          record: () => {
          },
          appStore: s
        });
        const state = dispatch2(s.state, {
          type: "setUi",
          ui,
          recordHistory
        });
      ...
      resolveAndCommitData: () => __async(null, null, function* () {
        const { config, state, dispatch: dispatch2, resolveComponentData: resolveComponentData2 } = get();
        walkAppState(
          state,
          config,
          (content) => content,
