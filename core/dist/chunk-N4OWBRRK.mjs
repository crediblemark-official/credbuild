...
var tidyState = (state2) => {
  return __spreadProps(__spreadValues({}, state2), {
    ui: __spreadProps(__spreadValues({}, state2.ui), {
      field: __spreadProps(__spreadValues({}, state2.ui.field), {
        focus: null
      })
    })
  });
  ...
  return {
    ...
    forward: () => {
      ...
      if (history.hasFuture()) {
        const state2 = (_a = history.nextHistory()) == null ? void 0 : _a.state;
        dispatch({ type: "set", state: state2 ? tidyState(state2) : {} });
        set({ history: __spreadProps(__spreadValues({}, history), { index: history.index + 1 }) });
      }
    },
...
var createPermissionsSlice = (set, get) => {
  const resolvePermissions = (..._0) => __async(null, [..._0], function* (params = {}, force) {
    const { state: state2, permissions, config } = get();
    ...
    if (item) {
      ...
      flattenData(state2, config).filter((item2) => item2.type === type).map((item2) => __async(null, null, function* () {
        yield resolvePermissionsForItem(item2, force);
      }));
    } else if (root) {
      resolvePermissionsForRoot(force);
    } else {
      flattenData(state2, config).map((item2) => __async(null, null, function* () {
        yield resolvePermissionsForItem(item2, force);
      }));
    }
  });
...
var useRegisterFieldsSlice = (appStore, id) => {
  const resolveFields = useCallback(
    (reset) => __async(null, null, function* () {
      ...
      const { getComponentConfig, state: state2 } = appStore.getState();
      ...
      if (resolver) {
        ...
        const newFields = yield resolver(componentData, {
          ...
          lastData,
          appState: makeStatePublic(state2),
          parent
        });
  ...
  subscribeWithSelector2((set, get) => {
    ...
    return __spreadProps(__spreadValues({
      ...
      setUi: (ui, recordHistory) => set((s) => {
        ...
        const state2 = dispatch(s.state, {
          type: "setUi",
          ui,
          recordHistory
        });
        const selectedItem = state2.ui.itemSelector ? getItem(state2.ui.itemSelector, state2) : null;
        return __spreadProps(__spreadValues({}, s), { state: state2, selectedItem });
      }),
      resolveComponentData: (componentData, trigger) => __async(null, null, function* () {
        var _a2, _b2;
        const { config, metadata, setComponentLoading, permissions, state: state2 } = get();
        const componentId = "id" in componentData.props ? componentData.props.id : "root";
        const parentId = (_a2 = state2.indexes.nodes[componentId]) == null ? void 0 : _a2.parentId;
        const parentNode = parentId ? state2.indexes.nodes[parentId] : null;
      ...
      resolveAndCommitData: () => __async(null, null, function* () {
        const { config, state: state2, dispatch, resolveComponentData: resolveComponentData2 } = get();
        walkAppState(
          state2,
          config,
          (content) => content,
          (childItem, path) => {
            ...
            resolveComponentData2(childItem, "load").then((resolved) => {
              const { state: state3 } = get();
              const node = state3.indexes.nodes[resolved.node.props.id];
              if (node && resolved.didChange) {
                if (resolved.node.props.id === "root") {
                  ...
                  const parentZone = state3.indexes.zones[zoneCompound];
...
var useResetAutoZoom = (frameRef) => {
  ...
  const resetAutoZoom = useCallback2(
    (options) => {
      const { state: state2, zoomConfig, setZoomConfig } = appStoreApi.getState();
      const { viewports } = state2.ui;
